...
internal static partial class Day22
{
	...
	private static int Part3()
	{
		...
		int result = DijkstraSearch(false);
		timer.Stop();
	...
	private static int Part4()
	{
		...
		Stopwatch timer = Stopwatch.StartNew();
		int result = DijkstraSearch(true);
		timer.Stop();
		Console.WriteLine("Time: {0} ms", timer.ElapsedMilliseconds);
		return result;
	}
	...
	{
		PriorityQueue<SearchNode> toVisit = new PriorityQueue<SearchNode>(comparer: SearchNode.CostComparer);
	...
	private static void AddNeighbor(SearchNode parent, Spell spell, int cost, PriorityQueue<SearchNode> toVisit, bool part2)
	{
